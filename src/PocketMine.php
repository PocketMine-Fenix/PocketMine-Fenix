...
JIT_WARNING
	...
	function server(){
		...
		$composerGitHash = InstalledVersions::getReference('pocketmine-fenix/pocketmine-fenix');
		...
		if(count(getopt("", [BootstrapOptions::VERSION])) > 0){
			...
			printf("Multi-version support: accepting client protocols from %d up to %d (%d versions supported)\n", min(ProtocolInfo::ACCEPTED_PROTOCOL), ProtocolInfo::CURRENT_PROTOCOL, count(ProtocolInfo::ACCEPTED_PROTOCOL));
			exit(0);
		}
