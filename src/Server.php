...
class Server{
	 ...
	 */
	private ?array $acceptedProtocols = null;

	/**
	 * Returns the list of client protocol versions allowed to join the server.
	 *
	 * @return int[]
	 * @phpstan-return list<int>
	 */
	public function getAcceptedProtocols() : array{
		if($this->acceptedProtocols === null){
			$configured = $this->configGroup->getProperty(Yml::NETWORK_ACCEPTED_PROTOCOLS, []);
			if(!is_array($configured)){
				throw new \InvalidArgumentException("network.accepted-protocols must be a list of protocol version numbers");
			}

			$accepted = [];
			foreach(Utils::promoteKeys($configured) as $protocol){
				if(!is_numeric($protocol)){
					$this->logger->warning("Ignoring non-numeric entry in network.accepted-protocols: " . print_r($protocol, true));
					continue;
				}
				$protocol = (int) $protocol;
				if(!in_array($protocol, ProtocolInfo::ACCEPTED_PROTOCOL, true)){
					$this->logger->warning("Ignoring unsupported protocol version $protocol in network.accepted-protocols (not in ProtocolInfo::ACCEPTED_PROTOCOL)");
					continue;
				}
				$accepted[] = $protocol;
			}
			...
			if(count($accepted) === 0){
				...
				sort($accepted);
				$this->logger->info("Accepting client connections from the following protocols only: " . implode(", ", $accepted));
			}

			$this->acceptedProtocols = $accepted;
		}
